...
/**
 * POST /api/setup/create-supabase-tables
 * Creates all necessary Supabase tables
 *
 * This endpoint:
 * 1. Connects to Supabase PostgreSQL
 * 2. Creates orders, projects, and clients tables
 * 3. Inserts test data for aprossine@gmail.com
 *
 * Authorization: Requires API_SETUP_TOKEN in Authorization header
 */

export default defineEventHandler(async (event) => {
  try {
    // Security: Check for setup token
    const authHeader = getHeader(event, 'authorization')
    const setupToken = process.env.API_SETUP_TOKEN

    if (!setupToken || authHeader !== `Bearer ${setupToken}`) {
      setResponseStatus(event, 401)
      return {
        success: false,
        error: 'Unauthorized. Set API_SETUP_TOKEN in .env.local',
      }
    }
    ...
    if (!databaseUrl) {
      ...
      return {
        success: false,
        error: 'DATABASE_URL not configured. Add to .env.local: DATABASE_URL=postgresql://postgres:PASSWORD@wkxwjasgyulakiyclipb.supabase.co:5432/postgres',
      }
    }
    ...
    return {
      success: false,
      error: error.message,
      hint: 'Make sure DATABASE_URL is set in .env.local with your Supabase database password',
    }
  }
})
