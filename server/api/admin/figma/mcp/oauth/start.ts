...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!clientId) {
      throw createError({
        statusCode: 500,
        statusMessage: 'FIGMA_MCP_CLIENT_ID not configured and no registration endpoint available',
      })
    }
    ...
    const message = error?.statusMessage || error?.message || 'Failed to start MCP OAuth'
    throw createError({ statusCode: error?.statusCode || 500, statusMessage: message })
  }
})
