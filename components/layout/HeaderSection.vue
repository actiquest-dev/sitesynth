<template>
  ...
  >
    <div
      class="max-w-[1248px] mx-auto px-6 flex items-center justify-between h-20"
    >
      <!-- Logo (always visible) -->
      ...
      <button
        @click="toggleMenu"
        class="cursor-pointer lg:hidden text-[#6E0015] text-2xl"
        aria-label="Open menu"
      >
        <div class="flex flex-col space-y-1">
          <span
            :class="[
              'block w-8 h-[3px] bg-white transition-transform duration-300',
              menuOpen ? 'rotate-45 translate-y-[7px]' : '',
            ]"
          ></span>
          <span
            :class="[
              'block w-8 h-[3px] bg-white transition-opacity duration-300',
              menuOpen ? 'opacity-0' : '',
            ]"
          ></span>
          <span
