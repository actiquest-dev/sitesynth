        ...
        <div v-for="(item, index) in navItems" :key="index" class="relative group">
            <template v-if="item.subItems">
                ...
                <NuxtLink :href="item.link"
                    class="hover:text-[#A620FF] font-medium transition-colors duration-1000 flex items-center space-x-1 group">
                            ...
                            <div>
                                <h3 class="text-white text-base font-semibold mb-1">
                                    <NuxtLink :href="subItem.link"
                                        class="hover:text-[#A620FF] transition-colors duration-300">{{ subItem.label
                                        }}</NuxtLink>
                                </h3>
                                <p v-if="subItem.description" class="text-gray-300 text-sm">{{ subItem.description }}
                                </p>
                            </div>
                        </li>
                    </ul>
            ...
            <!-- Single Link -->
            <template v-else>
                <NuxtLink :href="item.link" class="hover:text-[#A620FF] font-medium transition-colors duration-1000">
                    {{ item.label }}
                </NuxtLink>
            </template>
        </div>
...
const navItems = [
    { label: "Product", link: "#" },
    {
        ...
        subItems: [
            { imageSrc: "/Synth/assets/frame-1.svg", label: "Brand-Driven Product Strategy", description: "Align product design with your brand’s identity and goals.", link: "#" },
            { imageSrc: "/Synth/assets/frame-2.svg", label: "UX & Design Systems", description: "Build consistent, scalable, and user-friendly interfaces.", link: "#" },
